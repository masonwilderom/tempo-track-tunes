...
import { saveTrackCue, getTrackCues } from '@/lib/firebase';
...
const TrackCuePoints: React.FC<TrackCuePointsProps> = ({ trackId, playlistId, duration }) => {
  const [cuePoints, setCuePoints] = useState<CuePoint[]>([]);
  const [newCueName, setNewCueName] = useState('');
  const [newCueTime, setNewCueTime] = useState('');
  const [error, setError] = useState<string | null>(null);

  // Load existing cue points on mount
  useEffect(() => {
    const loadCues = async () => {
      try {
        const savedCues = await getTrackCues(trackId, playlistId);
        setCuePoints(savedCues);
      } catch (err) {
        console.error("Failed to load cue points:", err);
      }
    };
    loadCues();
  }, [trackId, playlistId]);
  ...
  const handleAddCue = async () => {
    ...
    try {
      await saveTrackCue(trackId, playlistId, {
        name: newCue.name,
